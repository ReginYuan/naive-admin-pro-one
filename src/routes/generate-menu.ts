...
export const generateMenu = (routes: RouteRecordRaw[]) => {
  ...
  for (const route of routes) {
    ...
    if (route.meta?.icon) currentMenu.icon = () => h(SideIcon, { icon: route.meta?.icon })

    if (route.children && route.children.length > 0) currentMenu.children = generateMenu(route.children)
