...
const getComponent = (component?: string) => {
  if (!component) return defineComponent.BlankView
...
const flatRouteData = (routes: RouteRecordRaw[]) => {
  ...
  for (const route of routes) {
    ...
    if (route.children && route.children.length > 0) flatRoutes.push(...flatRouteData(route.children))
  }
  return flatRoutes
}
...
const generator = (menuInfo: MenuInfo[], pid?: number) => {
  ...
  for (const currentMenu of currentMenus) {
    ...
    if (!currentRoute.children || currentRoute.children.length < 1) delete (currentRoute as RouteRecordRaw).children
